...
import { getAll, deleteRedirection, updateRedirection, createRedirection } from 'api/api-redirections';
...
const ActionCell = ({ item, editingId, onChange, onEditing, form }: any) => {
	...
	const onDeleteBtn = () => {
		...
		deleteRedirection(item.id).then((res)=>{
    ...
    const onSave = () => {
        setIsRequesting(true);
        updateRedirection(item.id, form.getFieldsValue()).then(res=>{
            setIsRequesting(false);
            onEditing('');
        }).catch(err=>{
            console.log(err);
...
const Index = ({ match }: any) => {
    ...
    const handleOk = () => {
        setIsRequesting(true);
        createRedirection(createForm.getFieldsValue()).then(res=>{
            setIsRequesting(false);
            loadTable();
    ...
    return (
        <>
            <Card title = "Redirections" actions = {[<div style = {{display:'flex'}}><Button type="primary" style = {{marginRight:24, marginLeft:'auto'}} onClick = {()=>{
